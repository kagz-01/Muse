...
export default function ArtifactExtractor({ onExtract }: { onExtract: (meta: ExtractedMetadata) => void }) {
  ...
  const simulateExtraction = () => {
    ...
    setTimeout(() => {
      ...
      if (inputType === 'link') {
        meta = {
          title: input.includes('x.com') ? "The Future of Digital Sovereignty" : "Aesthetic Brutalism in Modern Web",
        ...
        <div className="space-y-8 relative z-10">
          <div className="flex items-center justify-between">
            <h3 className="text-[10px] font-bold uppercase tracking-[0.4em] text-gray-500 flex items-center gap-3">
               <Sparkles size={14} className="text-canvas-primary" /> Multi-Signal Extraction Terminal
            </h3>
            <div className="flex items-center gap-2 p-1 bg-white/5 rounded-xl border border-white/5">
               {(['link', 'text', 'file'] as InputType[]).map(type => (
                 <button 
                   ...
                   className={`px-4 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all ${inputType === type ? 'bg-white text-black' : 'text-gray-500 hover:text-white'}`}
                 >
                    {type}
                 </button>
               ))}
          ...
          <div className="relative">
            {inputType === 'text' ? (
              ...
              <input 
                ...
                placeholder={inputType === 'link' ? "Paste social signal (X, Reddit, Notion, etc.)..." : "Drop file signal..."}
                disabled={isExtracting}
            ...
            {!isExtracting && (
              <button 
                ...
                className={`absolute bottom-6 right-6 w-12 h-12 bg-white text-black rounded-xl flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-xl ${inputType !== 'text' ? 'top-1/2 -translate-y-1/2' : ''}`}
              >
                <Sparkles size={20} />
              </button>
            )}
          ...
          {isExtracting && (
            <div className="space-y-4">
              {[
                { label: `Establishing connection to ${inputType} signal...`, active: step >= 1 },
                { label: 'Bypassing algorithm noise & sanitizing...', active: step >= 2 },
                { label: 'Synthesizing data type & recommending room...', active: step >= 3 }
              ].map((s, i) => (
        ...
        <div className="relative z-10 animate-in fade-in zoom-in-95 duration-500">
           <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                 <div className="px-3 py-1 bg-emerald-500/10 border border-emerald-500/30 rounded-lg text-[9px] font-bold uppercase tracking-widest text-emerald-500">Signal Extraction Complete</div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-12 gap-10">
              ...
              <div className={`${metadata.image ? 'md:col-span-8' : 'md:col-span-12'} flex flex-col justify-center`}>
                 <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-gray-500">
                       ...
                       {metadata.type === 'Raw Text' && <MessageSquare size={12} />}
                       {metadata.type}
                    </span>
                 </div>
                 <h4 className="text-3xl font-bold text-white mb-6 tracking-tight leading-tight">{metadata.title}</h4>
                 <p className="text-lg text-gray-400 font-serif italic leading-relaxed border-l-4 border-white/10 pl-8 mb-8">{metadata.summary}</p>
                 ...
                 {metadata.suggestedRoomId && (
                   <div className="mb-10 p-5 bg-canvas-primary/5 border border-canvas-primary/20 rounded-2xl flex items-center justify-between">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500">AI Recommendation</p>
                            <p className="text-xs font-bold text-white">Suggested Room: <span className="text-canvas-primary">{rooms.find(r => r.id === metadata.suggestedRoomId)?.name}</span></p>
                         </div>
                      </div>
                      <button className="text-[9px] font-bold uppercase tracking-widest text-canvas-primary hover:underline">Change Room</button>
                   </div>
                 )}
