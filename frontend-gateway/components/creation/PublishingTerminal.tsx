...
import { 
  Sparkles, Zap, Globe, Shield, Send, 
  Layers, GitCommit, ArrowRight, Check, X,
  Activity, Cpu, Lock
} from "lucide-preact";
  ...
  const handlePublish = () => {
    ...
    setTimeout(() => {
      publishThought({
        ...
        auraGradients: selectedThread?.customStyling?.auraGradients || [user.auraColor, '#10b981'],
        isImmutable
      });
      setContent('');
      ...
      <div className="relative z-10 space-y-12">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-8">
               <h4 className="text-[9px] font-bold uppercase tracking-widest text-gray-600">Select Synthesis Source</h4>
               <div className="space-y-4 max-h-[300px] overflow-y-auto pr-4 scrollbar-hide">
                  {threads.map(thread => (
                    <button 
                      key={thread.id}
                      onClick={() => setSelectedThreadId(thread.id)}
                      className={`w-full p-6 rounded-[2rem] border text-left transition-all ${selectedThreadId === thread.id ? 'bg-canvas-primary/10 border-canvas-primary shadow-xl' : 'bg-white/5 border-white/5 hover:border-white/20'}`}
                    >
                       <div className="flex items-center justify-between mb-2">
                          <span className="text-[9px] font-bold uppercase tracking-widest text-white">{thread.title}</span>
                          <GitCommit size={14} className="text-canvas-primary" />
                       </div>
                       <p className="text-[10px] text-gray-500 font-serif italic line-clamp-1">{thread.thesis}</p>
                    </button>
                  ))}
               </div>
            </div>
            ...
            {/* PUBLISHING CONTROLS */}
            <div className="space-y-8">
               <h4 className="text-[9px] font-bold uppercase tracking-widest text-gray-600">Sovereignty Options</h4>
               <div className="grid grid-cols-1 gap-4">
                  <button 
                    onClick={() => setIsImmutable(!isImmutable)}
                    className={`p-6 rounded-[2rem] border text-left transition-all flex items-center justify-between ${isImmutable ? 'bg-white/5 border-white/20' : 'bg-transparent border-white/5'}`}
                  >
                     <div>
                        <div className="text-[10px] font-bold uppercase tracking-widest text-white mb-1">Immutable Thought</div>
                        <p className="text-[9px] text-gray-600 uppercase tracking-widest">Seal this synthesis on the ledger</p>
                     </div>
                     {isImmutable ? <Shield size={20} className="text-canvas-primary" /> : <Lock size={20} className="text-gray-800" />}
                  </button>
                  <div className="p-6 rounded-[2rem] bg-white/[0.02] border border-white/5 text-center">
                     <p className="text-[9px] font-bold uppercase tracking-widest text-gray-600 mb-2">Final Resonance Predictor</p>
                     <div className="text-2xl font-bold text-white">84% Predicted Impact</div>
                  </div>
               </div>
            </div>
         ...
         <div className="space-y-6">
            <h4 className="text-[9px] font-bold uppercase tracking-widest text-gray-600">Final Synthesis</h4>
            <textarea 
              value={content}
         ...
         <div className="flex items-center gap-8 pt-8 border-t border-white/5">
            <div className="flex-1">
               <div className="flex items-center gap-4 mb-2">
                  <Cpu size={16} className="text-canvas-primary animate-spin-slow" />
                  <span className="text-[9px] font-bold uppercase tracking-widest text-gray-500">Synthesizing Collective Aura...</span>
               </div>
               <p className="text-[10px] text-gray-600 font-serif italic">This thought will be broadcasted with your signature aura to all active collective nodes.</p>
            </div>
            <button 
            ...
            >
               {isPublishing ? <Activity size={18} className="animate-pulse" /> : <Globe size={18} />}
