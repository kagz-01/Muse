...
interface Props {
  onSubmit: (text: string, tone: DialogueTone) => void;
  initialText?: string;
}

export default function ThoughtfulComposer(
  { onSubmit, initialText = "" }: Props,
) {
  ...
  return (
    <div className="bg-linear-to-b from-[#1c1c1c] to-canvas-bg-dark border border-white/10 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden transition-all duration-500">
      {/* Dynamic Glow */}
      <div
        className="absolute -top-32 -right-32 w-80 h-80 rounded-full blur-[100px] pointer-events-none transition-colors duration-1000 opacity-20"
        style={{
          backgroundColor: mode === "Ask"
            ? "#f59e0b"
            : mode === "Build"
            ? "#10b981"
            : "#6366f1",
      ...
      <textarea
        ...
        placeholder={mode === "Reflect"
          ? "Share how this resonates with your internal landscape..."
          : mode === "Build"
          ? "Add a structural layer to this idea..."
      ...
      <div className="flex items-center justify-between mt-6 pt-8 border-t border-white/5 relative z-10">
        ...
        >
          {mode === "Ask"
            ...
            ? "Add Perspective"
            : "Publish Reflection"}
          <Icons.Send size={14} className="ml-1" />
        </button>
      </div>
    </div>
  );
