...
import { Sparkles, MessageSquare, Activity, UserPlus, ChevronRight } from "lucide-preact";

export default function AuraCard({ collaborator }: { collaborator: Collaborator }) {
  return (
      ...
      <div className="relative z-10 flex flex-col h-full justify-between gap-10">
        ...
        <div className="flex justify-between items-start">
          <div className="relative">
             <img src={collaborator.avatar} className="w-20 h-20 rounded-[2rem] object-cover border-2 border-white/10 group-hover:border-white/20 transition-all" alt="" />
             <div className={`absolute -bottom-2 -right-2 w-6 h-6 rounded-lg bg-black border border-white/10 flex items-center justify-center text-[10px] font-bold`} style={{ color: collaborator.aura }}>
                {collaborator.status === 'Online' ? <Activity size={12} /> : collaborator.status === 'Deep Focus' ? <Sparkles size={12} /> : '•'}
             </div>
          </div>
          ...
          <div className="flex flex-col items-end">
             ...
             <span className={`text-[9px] font-bold uppercase tracking-[0.2em]`} style={{ color: collaborator.aura }}>
                {collaborator.status}
             </span>
          </div>
        </div>
        ...
        <div>
          <h3 className="text-2xl font-bold text-white mb-3 tracking-tight group-hover:text-canvas-primary transition-colors">{collaborator.name}</h3>
          ...
          <div className="flex flex-wrap gap-2">
            {collaborator.sharedThemes.map(theme => (
              <span key={theme} className="px-4 py-1.5 bg-white/5 border border-white/5 rounded-full text-[9px] font-bold uppercase tracking-widest text-gray-600">
                {theme}
              </span>
            ))}
          </div>
