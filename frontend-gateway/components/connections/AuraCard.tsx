...
export default function AuraCard(
  ...
  return (
    ...
    <div className="group relative bg-white/[0.03] border border-white/5 rounded-[2.5rem] p-8 transition-all hover:bg-white/[0.05] hover:border-white/10 hover:shadow-2xl flex flex-col h-full">
      ...
      <div className="relative z-10 flex flex-col h-full justify-between gap-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="button" className="w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-gray-500 hover:text-white hover:bg-white/10 transition-all cursor-pointer">
    ...
    {showNeuralLink && (
      <NeuralLinkModal collaborator={collaborator} onClose={() => setShowNeuralLink(false)} />
    )}
    </>
  );
}
