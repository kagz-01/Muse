import { Shield, MessageCircle, Zap, ExternalLink, Sparkles } from "lucide-preact";
...
export default function CollaboratorCard({ collaborator }: Props) {
  ...
  return (
    <div 
      className="group relative bg-[#0d0d0d] border border-white/5 rounded-[2.5rem] p-8 transition-all duration-500 hover:bg-white/[0.04] hover:border-indigo-500/30 card-glow glow-indigo shadow-2xl overflow-hidden"
    >
      <div className="relative z-10 flex items-start justify-between mb-8">
        <div className="relative">
          ...
          <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-[#0d0d0d] ${statusColors[collaborator.status]} shadow-lg shadow-black/50`} />
        </div>

        <div className="flex gap-2">
           <button type="button" className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:bg-indigo-500/20 hover:border-indigo-500/30 transition-all cursor-pointer">
              <MessageCircle size={18} />
           </button>
           <button type="button" className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all cursor-pointer">
      ...
      <div className="relative z-10 mb-6">
        <h4 className="text-2xl font-bold text-white mb-2 group-hover:text-indigo-400 transition-colors tracking-tight">{collaborator.name}</h4>
      ...
      <div className="relative z-10 pt-6 border-t border-white/5 flex flex-wrap gap-2">
        {collaborator.sharedThemes.map(theme => (
          <span key={theme} className="text-[9px] font-bold text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-3 py-1.5 rounded-lg uppercase tracking-widest flex items-center gap-2">
