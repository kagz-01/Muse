...
export default function PortraitCard(
  { user, activeRooms, soloMode, className = "" }: PortraitCardProps,
) {
  ...
  return (
    <div className={`w-full max-w-sm perspective-distant ${className}`}>
      <div className="bg-linear-to-b from-white/[0.08] to-[#0a0a0a] rounded-[2.5rem] border border-white/10 overflow-hidden shadow-[0_40px_80px_rgba(0,0,0,0.8)] transform-gpu transition-all duration-700 hover:rotate-y-2 hover:rotate-x-2 hover:-translate-y-2 relative group">
        <div className="absolute -top-20 -right-20 w-64 h-64 bg-indigo-500/10 blur-[100px] rounded-full pointer-events-none group-hover:bg-indigo-500/20 transition-colors duration-700" />
        <div className="absolute -bottom-20 -left-20 w-64 h-64 bg-emerald-500/10 blur-[100px] rounded-full pointer-events-none group-hover:bg-emerald-500/20 transition-colors duration-700" />
        ...
        <div className="h-40 bg-linear-to-br from-indigo-500/30 via-emerald-500/10 to-transparent relative border-b border-white/5 flex justify-center">
          ...
          <div className="absolute -bottom-12 w-24 h-24 rounded-full bg-[#0a0a0a] border-[4px] border-[#0a0a0a] overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.4)] flex items-center justify-center text-4xl font-serif text-gray-500 z-10 group-hover:scale-105 transition-transform duration-500">
            {user.avatarUrl
              ? (
                <img
                  src={user.avatarUrl}
          ...
          <div className="mb-10 flex flex-col items-center">
            ...
            <div className="flex flex-wrap justify-center gap-2.5">
              {activeRooms.map((room) => (
                <div
                  key={room.id}
                  className="bg-white/5 border border-white/8 px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest text-gray-300 hover:bg-white/10 hover:border-white/20 transition-all cursor-default"
                >
                  {room.name}
                </div>
              ))}
