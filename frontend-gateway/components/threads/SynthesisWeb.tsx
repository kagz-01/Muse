...
import { Layers, Sparkles, Hash, GitCommit } from "lucide-preact";
      ...
      <div className="relative z-10 space-y-10">
         <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 px-4 py-1.5 bg-emerald-500/10 border border-emerald-500/30 rounded-xl">
               ...
               <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-500">{thread.synthesisScore}% Resonance</span>
            </div>
         </div>
         ...
         <div className="flex flex-col md:flex-row items-center justify-center gap-12 py-10">
            {sourceRooms.map((room, i) => (
              <div key={room.id} className="relative group">
                 <div className={`w-32 h-32 rounded-[2.5rem] border border-white/10 bg-black/40 flex items-center justify-center overflow-hidden transition-all duration-700 group-hover:scale-110 group-hover:border-canvas-primary/40 shadow-2xl`}>
                    {room.coverImage ? (
                      <img src={room.coverImage} className="w-full h-full object-cover opacity-40 group-hover:opacity-60 transition-opacity" alt="" />
                    ) : (
                      <div className="w-full h-full bg-white/5" />
                    )}
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                       <Hash size={20} className="text-gray-500 mb-2" />
                       <p className="text-[9px] font-bold uppercase tracking-widest text-white leading-tight">{room.name}</p>
                    </div>
                 </div>
         ...
         <div className="p-8 bg-white/2 border border-white/5 rounded-[2.5rem] backdrop-blur-sm">
            <h4 className="text-[9px] font-bold uppercase tracking-widest text-gray-600 mb-4">Master Thesis Pattern</h4>
