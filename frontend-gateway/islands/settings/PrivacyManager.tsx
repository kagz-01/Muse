import { Check, Eye, EyeOff, Globe, ShieldCheck, X } from "lucide-preact";
import { togglePublicSetting, User, userSignal } from "../../signals/user.ts";
...
export default function PrivacyManager(
  { isOpen, onClose }: PrivacyManagerProps,
) {
  ...
  const settings = [
    {
      ...
      desc: "Allow others to see your name, bio, and avatar.",
    },
    {
      key: "showLocation",
      label: "Display Location",
      desc: "Show your current city/digital coordinate.",
    },
    {
      key: "showRooms",
      label: "Shared Rooms",
      desc: "List your public rooms on your community portrait.",
    },
    {
      key: "showThreads",
      label: "Active Dialogue",
      desc: "Show your participation in community circles.",
    },
    {
      key: "showInsights",
      label: "Resonance Metrics",
      desc: "Share your thematic resonance and connection stats.",
    },
  ] as const;
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 animate-in fade-in duration-300">
      <div
        className="absolute inset-0 bg-canvas-bg-dark/90 backdrop-blur-3xl"
        onClick={onClose}
      />
      ...
      <div className="relative w-full max-w-lg bg-[#111111] border border-white/5 rounded-[2.5rem] p-10 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500">
        ...
        <div className="space-y-6">
          {settings.map((s) => {
            const isActive =
              user.publicSettings[s.key as keyof User["publicSettings"]];
            return (
              <div
                key={s.key}
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() =>
                    togglePublicSetting(s.key as keyof User["publicSettings"])}
                  className={`w-14 h-8 rounded-full flex items-center px-1 transition-all duration-500 cursor-pointer ${
                    isActive ? "bg-emerald-600" : "bg-gray-800"
                  }`}
                >
                  <div
                    className={`w-6 h-6 rounded-full bg-white flex items-center justify-center shadow-lg transition-transform duration-500 ${
