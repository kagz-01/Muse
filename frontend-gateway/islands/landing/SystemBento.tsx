import { Layout, Sparkles, BookOpen, Network, Shield, Zap, Target } from "lucide-preact";
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 px-4">
        <div className="max-w-2xl">
          ...
          <h3 className="text-4xl md:text-6xl font-bold tracking-tight text-[var(--muse-text)] leading-tight transition-colors duration-300">
            Engineered for <span className="text-[var(--muse-muted)] italic font-serif">Intention.</span>
          </h3>
        </div>
        ...
        {features.map((f) => (
          ...
          >
            ...
            <div className="relative z-10 flex flex-col h-full justify-between gap-12">
              <div className={`w-14 h-14 rounded-2xl bg-[var(--muse-surface-soft)] flex items-center justify-center ${f.color} transition-all duration-300`}>
                <f.icon size={28} />
              </div>
              
              <div>
                <h4 className="text-2xl font-bold text-[var(--muse-text)] mb-4 tracking-tight transition-colors duration-300">{f.title}</h4>
      ...
      <div className="bg-[var(--muse-surface)] border border-[var(--muse-border)] rounded-[3rem] p-8 md:p-16 flex flex-wrap justify-between gap-12 transition-all duration-300">
        {[
          ...
          <div key={s.label} className="flex items-center gap-6">
             ...
             <div>
                <p className="text-2xl font-bold text-[var(--muse-text)] font-mono transition-colors duration-300">{s.val}</p>
                <p className="text-[9px] font-bold uppercase tracking-widest text-[var(--muse-muted)] mt-1 transition-colors duration-300">{s.label}</p>
             </div>
          </div>
        ))}
      </div>
