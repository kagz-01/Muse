...
import { Activity, Calendar, TrendingUp } from "lucide-preact";
      ...
      <div class="space-y-2">
        ...
        <div class="grid grid-cols-7 gap-1">
          {daysOfWeek.map((day) => (
            <div
              key={day}
              class="text-xs text-white/50 text-center font-medium py-2"
            >
              {day}
            </div>
          ))}
...
export function ActivityTimeline(
  { entries, days = 30 }: ActivityTimelineProps,
) {
  const timeline = buildTimeline(entries, days);
    ...
    <div class="space-y-4">
        ...
        {timeline.map((day, i) => (
          ...
          >
            <div
              ...
              style={{
                height: `${(day.entries / maxEntries) * 100}%` || "4px",
              }}
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <p class="text-lg font-semibold text-white">
            {timeline.reduce((max, t) => (t.entries > max.entries ? t : max))
              .entries} entries
          </p>
        </div>
        <div>
...
function calculateStats(timeline: TimelineEntry[]) {
  const maxDay = timeline.reduce((
    max,
    t,
  ) => (t.entries > max.entries ? t : max));
