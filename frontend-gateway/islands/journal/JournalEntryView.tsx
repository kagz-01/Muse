...
import { setAmbientGlow, setResonanceMode } from "../../signals/resonance.ts";
...
export default function JournalEntryView({ entryId }: { entryId: string }) {
  ...
  const entry = useMemo(() => entries.find((e) => e.id === entryId), [
    entries,
    entryId,
  ]);
  ...
  const handleCommitThought = async () => {
    ...
    try {
      ...
      setAiResponses((prev) => ({
        ...prev,
        [blockIndex]: data.question || "What comes next?",
      }));
    } catch (err) {
  ...
  if (!entry) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#0a0a0a]">
        ...
        <a
          href="/journal"
          className="text-gray-400 hover:text-white text-sm mt-4 underline"
        >
          Back to Journal
        </a>
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0a0a0a] relative overflow-hidden">
      <div
        ...
        style={{
          background:
            `radial-gradient(circle at 50% 0%, ${cfg.color}, transparent 60%)`,
        }}
      />
            ...
            <div className="flex gap-4 animate-in fade-in duration-500">
              ...
              <div className="flex flex-col gap-2">
                ...
                <p className="text-gray-300 font-serif italic leading-relaxed text-lg">
                  The session is open. What's on your mind today? Let's break it
                  down.
                </p>
              </div>
            </div>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="bg-indigo-500/[0.02] border border-indigo-500/10 rounded-2xl rounded-tl-sm px-6 py-4 max-w-[85%] shadow-xl relative overflow-hidden">
                        ...
                        <div className="mt-4 flex gap-2">
                          <span className="px-2 py-1 rounded bg-white/5 text-[9px] uppercase tracking-widest text-gray-400 border border-white/5 hover:bg-white/10 cursor-pointer transition-colors">
                            Answer inline
                          </span>
                          <span className="px-2 py-1 rounded bg-white/5 text-[9px] uppercase tracking-widest text-gray-400 border border-white/5 hover:bg-white/10 cursor-pointer transition-colors">
                            Extract tags
              ...
              <div className="flex gap-4 animate-in fade-in duration-300">
                ...
                <div className="flex flex-col gap-2 justify-center">
                  <div className="flex gap-1.5 px-4 py-3 bg-indigo-500/[0.02] border border-indigo-500/10 rounded-2xl rounded-tl-sm w-fit">
                    <div
                      className="w-1.5 h-1.5 rounded-full bg-indigo-500/50 animate-bounce"
            ...
            {floatingContext.length > 0
              ? (
                <div className="space-y-4">
                  <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-4">
                    Resonating with your input:
                  </p>
                  {floatingContext.map((item) => (
                    <div
                      key={item.id}
                      className="p-4 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-canvas-primary/30 transition-all cursor-pointer animate-in fade-in slide-in-from-right-4"
                    >
                      ...
                      <p className="text-sm text-gray-300 font-serif line-clamp-2">
                        {item.title}
                      </p>
                    </div>
                  ))}
              ...
              : (
                <div className="flex flex-col items-center justify-center py-12 text-center opacity-50">
                  <Icons.Radar
                    size={32}
                    className="text-gray-600 mb-4 animate-[spin-slow_10s_linear_infinite]"
                  />
                  <p className="text-xs text-gray-500 italic font-serif">
                    Awaiting semantic resonance...<br />Start typing to surface
                    connections.
                  </p>
                </div>
              )}
          ...
          <div className="pt-8 border-t border-white/5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/5">
                <p className="text-[9px] uppercase tracking-widest font-bold text-gray-500 mb-1">
                  Words
                </p>
