...
export default function JournalEntryView({ entryId }: { entryId: string }) {
  ...
  const [aiInsights, setAiInsights] = useState<{ summary?: string; status?: string } | null>(null);
  const [blockchainResult, setBlockchainResult] = useState<{ solana_transaction_id: string; arweave_hash: string } | null>(null);
    ...
    <div className="min-h-screen flex flex-col bg-[#0a0a0a] relative">
      ...
      <header className="sticky top-0 z-30 px-6 md:px-10 py-5 flex items-center justify-between border-b border-white/5 backdrop-blur-2xl bg-[#0a0a0a]/80 shadow-2xl">
        <div className="flex items-center gap-6">
          <a href="/journal" className="w-11 h-11 rounded-2xl border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/5 hover:border-white/20 transition-all shadow-lg">
            <ArrowLeft size={20} />
          </a>
          ...
          <div className="relative">
            ...
            {showMoodPicker && (
              <div className="absolute top-full left-0 mt-4 bg-[#151515] border border-white/10 rounded-[2.5rem] p-5 shadow-2xl z-40 w-80 animate-in fade-in slide-in-from-top-2 backdrop-blur-3xl">
                <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-gray-500 mb-5 px-1">How are you feeling?</p>
                <div className="grid grid-cols-2 gap-2.5 mb-5">
                  {moods.filter(([m]) => m !== 'custom').map(([m, c]) => (
                    <button key={m} type="button" onClick={() => { setMood(m as JournalMood); setShowMoodPicker(false); }}
                      className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold cursor-pointer transition-all text-left ${mood === m ? 'text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                      style={mood === m ? { backgroundColor: c.color + '25', borderColor: c.color + '40', borderWidth: '1px' } : {}}
                    >
                      <span className="text-xl leading-none">{c.emoji}</span> {c.label}
                    </button>
                  ))}
                </div>
                <div className="border-t border-white/5 pt-5">
                   <p className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.25em] mb-3 px-1">Custom Mood</p>
                   <input
        ...
        <div className="flex items-center gap-4">
          <div className="hidden lg:flex items-center gap-1.5 bg-white/5 border border-white/10 rounded-2xl p-1.5">
            {(Object.entries(fontSizes) as [FontSize, typeof fontSizes[FontSize]][]).map(([size, config]) => (
              <button key={size} type="button" onClick={() => setFontSize(size)}
                className={`w-10 h-10 rounded-xl flex items-center justify-center text-[11px] font-bold transition-all cursor-pointer ${fontSize === size ? 'bg-white text-black shadow-lg' : 'text-gray-500 hover:text-white'}`}>
                {config.label}
              </button>
            ))}
          </div>
          ...
          <button onClick={() => toggleFavoriteJournal(entry.id)}
            type="button"
            className={`w-11 h-11 rounded-2xl border flex items-center justify-center transition-all cursor-pointer shadow-lg ${entry.isFavorited ? 'border-amber-500/40 bg-amber-500/10 text-amber-500' : 'border-white/10 text-gray-600 hover:text-white hover:bg-white/5'}`}>
            <Star size={20} fill={entry.isFavorited ? 'currentColor' : 'transparent'} />
          </button>
          ...
          <div className="flex items-center gap-5 font-bold text-[10px] uppercase tracking-[0.25em]">
            {saved ? (
              <span className="text-gray-600 flex items-center gap-2.5">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> {lastSavedTime ? formatTime(lastSavedTime) : 'Synced'}
              </span>
            ) : (
      ...
      {showDelete && (
        <div className="bg-rose-500/5 border-b border-rose-500/10 px-6 md:px-10 py-6 flex items-center justify-between relative z-20 animate-in slide-in-from-top-2 duration-300 backdrop-blur-md">
          <div className="flex items-center gap-4 text-[11px] font-bold text-rose-300 uppercase tracking-[0.25em]">
            <AlertTriangle size={20} className="text-rose-500" /> Confirm permanent deletion.
          </div>
          <div className="flex items-center gap-6">
            <button onClick={() => setShowDelete(false)} type="button" className="text-[11px] font-bold uppercase tracking-widest text-gray-500 hover:text-white transition-colors cursor-pointer">Cancel</button>
            <button onClick={handleDelete} type="button" className="px-8 py-3 bg-rose-500 hover:bg-rose-600 text-white text-[11px] font-bold uppercase tracking-[0.3em] rounded-2xl shadow-[0_10px_30px_rgba(244,63,94,0.3)] transition-all cursor-pointer">Purge Entry</button>
          </div>
        </div>
      )}
        ...
        <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            <div className="flex items-center gap-3 mb-4">
               ...
               <p className="text-[12px] font-bold uppercase tracking-[0.4em] text-white" style={{ color: cfg.color }}>
                 {formatDate(entry.createdAt)}
               </p>
            </div>
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-white/90">
               Contemplation <span className="text-gray-700">#{entryId.slice(0, 4)}</span>
            </h2>
          </div>
          <div className="flex items-center gap-4">
            <button 
              ...
              type="button"
              className={`flex items-center gap-3 group text-[11px] font-bold uppercase tracking-[0.2em] transition-all cursor-pointer shadow-xl px-6 py-3.5 rounded-2xl border border-white/10 ${isProcessing ? 'opacity-50 cursor-not-allowed' : 'hover:text-white hover:bg-white/5 hover:border-white/20'}`}
            >
              <Sparkles size={16} className={isProcessing ? "animate-spin text-indigo-400" : "group-hover:rotate-12 transition-transform text-indigo-400"} />
            ...
            <button onClick={() => setShowPrompt(!showPrompt)}
              ...
              <Book size={16} className="group-hover:rotate-12 transition-transform text-violet-400" />
              Prompt Engine
            </button>
          </div>
        </div>
            ...
            {aiInsights && (
              <div className="bg-[#111] border border-indigo-500/20 rounded-[2.5rem] p-8 card-glow glow-indigo relative overflow-hidden">
                ...
                <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-indigo-400 mb-5 flex items-center gap-2">
                   <div className="w-1.5 h-1.5 rounded-full bg-indigo-400" /> AI Semantic Analysis
                </p>
                <p className="text-gray-300 font-serif italic text-lg leading-relaxed relative z-10">
                  "{aiInsights.summary || aiInsights.status || "Your neural patterns have been successfully mapped to the collective consciousness."}"
                </p>
              </div>
            )}
            {blockchainResult && (
              <div className="bg-[#111] border border-violet-500/20 rounded-[2.5rem] p-8 card-glow glow-violet relative overflow-hidden">
                ...
                <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-violet-400 mb-5 flex items-center gap-2">
                   <div className="w-1.5 h-1.5 rounded-full bg-violet-400" /> Immutable Resonance
                </p>
                <div className="space-y-4 relative z-10">
                  <div className="flex flex-col gap-2">
                    <p className="text-[9px] font-bold text-gray-600 uppercase tracking-widest">Transaction ID</p>
                    <p className="text-[10px] text-gray-400 font-mono bg-white/5 p-2 rounded-lg truncate">{blockchainResult.solana_transaction_id}</p>
                  </div>
