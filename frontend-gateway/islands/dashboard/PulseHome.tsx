...
export default function PulseHome() {
  ...
  const mirrorStats = [
    { label: 'Artifacts', value: weekItems.length, icon: Layers, color: 'text-canvas-primary' },
    { label: 'Reflections', value: weekEntries.length, icon: BookOpen, color: 'text-emerald-400' },
    { label: 'Rooms Active', value: new Set(weekItems.map(i => i.roomId)).size, icon: BarChart2, color: 'text-amber-400' },
  ];
        ...
        <div className="relative z-10 grid gap-12 lg:grid-cols-[1.2fr_0.8fr] items-center">
          <div>
            ...
            <h1 className="text-5xl md:text-7xl font-bold tracking-tight leading-[0.95] text-white">
              {greeting}, <span className="text-gray-600 italic font-serif">{user?.name?.split(' ')[0] || "Creator"}</span>.
            ...
            <p className="mt-8 max-w-xl text-gray-400 text-lg md:text-xl leading-relaxed font-serif italic opacity-80">
              The platform is reflecting your semantic patterns. You have maintained a <span className="text-white font-sans font-bold not-italic">{streak} day</span> reflection streak.
            </p>
            ...
            <div className="mt-10 flex flex-wrap items-center gap-4">
              ...
              >
                Enter Vault <FolderOpen size={16} />
              </button>
            </div>
          </div>
          ...
          <div className="bg-black/40 backdrop-blur-2xl rounded-[2.5rem] border border-white/10 p-8 space-y-8">
            <div className="flex items-center justify-between border-b border-white/5 pb-6">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-bold uppercase tracking-[0.2em] text-white">Mirror Insights</span>
              </div>
              <a href="/mirror" className="text-[10px] font-bold uppercase tracking-widest text-gray-500 hover:text-white transition-colors">History</a>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {mirrorStats.map(stat => (
                <div key={stat.label} className="text-center">
                  ...
                  <p className="mt-2 text-[8px] font-bold uppercase tracking-widest text-gray-500">{stat.label}</p>
                </div>
              ))}
            </div>

            <div className="p-6 rounded-3xl bg-white/[0.03] border border-white/5">
              <p className="text-[10px] font-bold uppercase tracking-widest text-canvas-primary mb-3">Pattern Detected</p>
              <p className="text-sm text-gray-300 leading-relaxed font-serif italic">
                {topRoom 
                  ? `Your focus on "${topRoom.name}" has intensified this week with ${roomCounts[topRoomId]} new artifacts.`
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between px-2">
            <h2 className="text-2xl font-bold tracking-tight text-white flex items-center gap-3">
              Recent Resonance <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            </h2>
            <p className="text-xs text-gray-500 uppercase tracking-widest">Latest Actions</p>
          </div>

          <div className="grid gap-4">
            {[
              { title: latestEntry ? getJournalTitle(latestEntry) : "Journal", detail: latestEntry ? `${latestEntry.wordCount} words` : "No entries", time: latestEntry ? formatRelativeTime(latestEntry.createdAt) : "", icon: BookOpen, href: "/journal" },
              { title: latestThread ? latestThread.title : "Threads", detail: latestThread ? `${latestThread.itemIds.length} items` : "No threads", time: latestThread ? formatRelativeTime(latestThread.createdAt) : "", icon: MessageSquare, href: "/threads" },
              { title: latestRoom ? latestRoom.name : "Rooms", detail: latestRoom ? `${latestRoom.count} total` : "No rooms", time: latestRoom ? formatRelativeTime(latestRoom.createdAt) : "", icon: FolderOpen, href: "/rooms" },
            ].map(item => (
              <a 
                key={item.title} 
                href={item.href}
              ...
              >
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">{item.detail} • {item.time}</p>
                  </div>
                </div>
        ...
        <div className="space-y-8">
          <div className="bg-white/[0.02] rounded-[2.5rem] border border-white/5 p-8">
            <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-3">
              <TrendingUp size={20} className="text-canvas-primary" /> Active Themes
            </h3>
            <div className="flex flex-wrap gap-2">
              {topThemes.map(theme => (
                <span key={theme} className="px-4 py-2 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-widest text-gray-300 hover:border-canvas-primary/50 transition-colors">
                  {theme}
                </span>
              ))}
            </div>
            <div className="mt-8 p-6 rounded-3xl bg-black/20 border border-white/5">
              <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">Soul Signal</p>
              <p className="text-sm text-gray-400 font-serif italic leading-relaxed">
                Your current focus suggests a high degree of pattern density in {topThemes[0] || "emergent topics"}.
              </p>
            </div>
          </div>
          ...
          <div className="bg-canvas-primary/10 rounded-[2.5rem] border border-canvas-primary/20 p-8 text-center">
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-canvas-primary mb-2">Total Intelligence</p>
            <p className="text-4xl font-bold text-white mb-2">{rooms.reduce((s, r) => s + r.count, 0)}</p>
            <p className="text-xs text-gray-500 uppercase tracking-widest">Anchored Artifacts</p>
          </div>
        </div>
