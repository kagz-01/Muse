  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{ background: `linear-gradient(to right, transparent, ${room.theme_color}, transparent)` }}
      />
      ...
      <div className="flex items-center justify-between mt-6 pt-4 border-t border-[var(--muse-border)]/50">
        <div className="flex gap-2">
          {room.tags && room.tags.slice(0, 2).map(tag => (
            <span key={tag} className="px-2.5 py-1 rounded-md bg-[var(--muse-bg)] border border-[var(--muse-border)] text-[10px] font-bold uppercase tracking-wider text-[var(--muse-muted)]">
              {tag}
            </span>
          ))}
