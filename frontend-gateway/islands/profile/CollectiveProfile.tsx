  ...
  return (
    <div className="pb-24 md:pb-10 min-h-screen bg-[#0a0a0a] relative overflow-hidden">
      ...
      <div 
        ...
        style={{ background: `radial-gradient(circle at center, ${user.auraColor} 0%, transparent 70%)` }}
      />
      ...
      <header className="relative w-full h-[50vh] min-h-[450px] flex flex-col items-center justify-center text-center p-6 pt-24">
         <div className="relative mb-10 group">
            ...
            <div className="w-32 h-32 md:w-40 md:h-40 rounded-full bg-black/40 border-4 border-white/10 flex items-center justify-center relative z-10 overflow-hidden shadow-3xl">
               <span className="text-4xl md:text-5xl font-bold text-white">{user.name[0]}</span>
               {/* AURA TYPE BADGE */}
         ...
         <h1 className="text-4xl md:text-6xl font-bold tracking-tight text-white mb-4">{user.name}</h1>
         <div className="flex items-center gap-6 mb-12">
            <div className="flex items-center gap-2 text-canvas-primary">
               <Flame size={16} className="animate-pulse" />
               <span className="text-xs font-bold uppercase tracking-widest">{user.cognitiveStreak} Day Streak</span>
            </div>
            <div className="h-4 w-px bg-white/10" />
            <div className="flex items-center gap-2 text-emerald-500">
               <Activity size={16} />
               <span className="text-xs font-bold uppercase tracking-widest">{user.resonance.resonanceScore}% Resonance</span>
            </div>
         </div>

         <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl w-full">
            {[
              { label: 'Intelligence Lineage', value: user.synthesisLineage.totalRooms, sub: 'Source Rooms', icon: Layers },
              ...
              <div key={stat.label} className="p-6 bg-white/5 border border-white/5 rounded-3xl backdrop-blur-md">
                 ...
                 <div className="text-[9px] font-bold uppercase tracking-widest text-gray-500">{stat.label}</div>
              </div>
            ))}
         </div>
      </header>
            ...
            <div className="lg:col-span-1 space-y-8">
               <section className="bg-white/[0.03] border border-white/10 rounded-[3rem] p-10">
                  <h3 className="text-[10px] font-bold uppercase tracking-[0.4em] text-gray-500 mb-8 flex items-center gap-3">
                     <Shield size={14} className="text-canvas-primary" /> Collective Soul
                  </h3>
                  <p className="text-lg text-gray-300 font-serif italic leading-relaxed mb-10">
                    "A master of raw forms and cognitive sovereignty. Your patterns focus on the intersection of brutalist honesty and stoic resilience."
                  </p>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {threads.map(thread => (
                    <div key={thread.id} className="group bg-[#111] border border-white/5 rounded-[3rem] overflow-hidden hover:border-white/20 transition-all duration-500 shadow-2xl">
                       ...
                       <div className="p-8">
                          <h4 className="text-xl font-bold text-white mb-4 group-hover:text-canvas-primary transition-colors">{thread.title}</h4>
                          <div className="flex items-center justify-between pt-6 border-t border-white/5">
                             <div className="flex items-center gap-3">
                                <span className="text-[9px] font-bold uppercase tracking-widest text-gray-700">{thread.resonanceMetrics.connections} Connections</span>
                             </div>
                             <ArrowUpRight size={16} className="text-gray-700 group-hover:text-white group-hover:translate-x-1 group-hover:-translate-y-1 transition-all" />
                          </div>
                       </div>
                    </div>
                  ))}
