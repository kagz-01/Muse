...
export default function ThoughtStream() {
  const [activeSynthesisNode, setActiveSynthesisNode] = useState<string | null>(null);
  const [activeLineageNode, setActiveLineageNode] = useState<string | null>(null);
  const [activeResonanceNode, setActiveResonanceNode] = useState<string | null>(null);
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
             ...
             <div key={pers.id} className="mb-12">
               ...
               <div className="relative bg-white/[0.03] border border-white/5 rounded-[3rem] p-10">
                 ...
                 <div className="mt-8 pt-6 border-t border-white/[0.03] flex flex-wrap items-center gap-8">
                  <button
                    type="button"
                    onClick={() => setActiveSynthesisNode(pers.id)}
                  ...
                  >
                    ...
                    <span className="text-[9px] text-gray-500 font-serif italic pl-6">Branch into Journal</span>
                  </button>
                  ...
                  <div className="flex items-center gap-1 bg-white/5 border border-white/10 rounded-xl p-1">
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); alignWithPerspective(pers.id); }}
                      className="group/btn flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/10 transition-colors cursor-pointer"
                    >
                      <Icons.ArrowUp size={14} className="text-gray-500 group-hover/btn:text-emerald-400 transition-colors" />
                      <span className="text-[10px] font-bold text-gray-400 group-hover/btn:text-emerald-400">{pers.alignCount}</span>
                    </button>
                    <div className="w-px h-4 bg-white/10" />
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); challengePerspective(pers.id); }}
                      className="group/btn flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/10 transition-colors cursor-pointer"
                    >
                      <Icons.ArrowDown size={14} className="text-gray-500 group-hover/btn:text-amber-400 transition-colors" />
                      <span className="text-[10px] font-bold text-gray-400 group-hover/btn:text-amber-400">{pers.challengeCount}</span>
                    </button>
                  </div>

                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); setActiveResonanceNode(pers.id); }}
                    className="group/btn flex flex-col items-start gap-1 p-2 rounded-xl hover:bg-white/5 transition-colors cursor-pointer"
                  >
                    <div className="flex items-center gap-2.5 text-[10px] font-bold uppercase tracking-widest text-white">
                      <Icons.Waves size={14} className="text-emerald-500 group-hover/btn:scale-110 transition-transform" />{" "}
                      Resonate
                    </div>
                    <span className="text-[9px] text-gray-500 font-serif italic pl-6">Inject thought to cluster</span>
                  </button>

                  <button
                    type="button"
                    onClick={() => setActiveLineageNode(pers.id)}
                    className="group/btn flex flex-col items-start gap-1 p-2 rounded-xl hover:bg-white/5 transition-colors cursor-pointer"
                  >
                    <div className="flex items-center gap-2.5 text-[10px] font-bold uppercase tracking-widest text-white">
                      <Icons.Network size={14} className="text-indigo-500 group-hover/btn:scale-110 transition-transform" />{" "}
                      Trace Chain
                    </div>
                    <span className="text-[9px] text-gray-500 font-serif italic pl-6">View lineage map</span>
                  </button>
                </div>
               </div>
             </div>
              ...
              <div className="p-8 bg-white/[0.02] border border-white/5 rounded-[2.5rem]">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest text-white">Theme: Digital Silence</span>
                  </div>
                  <span className="text-[9px] font-bold text-gray-500 bg-white/5 px-2 py-1 rounded-md">12 Nodes</span>
                </div>
                <div className="space-y-4">
                  <div className="pl-4 border-l-2 border-cyan-500/30">
                    <p className="text-sm font-serif italic text-gray-400">"The absence of notifications is the ultimate luxury."</p>
                    <p className="text-[8px] font-bold uppercase tracking-widest text-gray-600 mt-2">— Elena V.</p>
                  </div>
                  <div className="pl-4 border-l-2 border-cyan-500/30">
                    <p className="text-sm font-serif italic text-gray-400">"We must architect quiet spaces deliberately."</p>
                    <p className="text-[8px] font-bold uppercase tracking-widest text-gray-600 mt-2">— Marcus T.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-8 bg-white/[0.02] border border-white/5 rounded-[2.5rem]">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest text-white">Theme: Deep Work</span>
                  </div>
                  <span className="text-[9px] font-bold text-gray-500 bg-white/5 px-2 py-1 rounded-md">8 Nodes</span>
                </div>
                <div className="space-y-4">
                  <div className="pl-4 border-l-2 border-purple-500/30">
                    <p className="text-sm font-serif italic text-gray-400">"Focus is fractured by default. Brutalism restores it."</p>
                    <p className="text-[8px] font-bold uppercase tracking-widest text-gray-600 mt-2">— Sarah K.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                <div className="mt-8 pt-6 border-t border-white/[0.03] flex flex-wrap items-center gap-6">
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); setActiveSynthesisNode(pers.id); }}
                    className="group/btn flex flex-col items-start gap-1 p-2 -ml-2 rounded-xl hover:bg-white/5 transition-colors cursor-pointer"
                  >
                    <div className="flex items-center gap-2.5 text-[10px] font-bold uppercase tracking-widest text-white">
                      <Icons.GitBranch
                  ...
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); setActiveLineageNode(pers.id); }}
                    className="group/btn flex flex-col items-start gap-1 p-2 rounded-xl hover:bg-white/5 transition-colors cursor-pointer"
                  >
                    <div className="flex items-center gap-2.5 text-[10px] font-bold uppercase tracking-widest text-white">
                      <Icons.Network size={14} className="text-indigo-500 group-hover/btn:scale-110 transition-transform" />{" "}
      ...
      {activeSynthesisNode && (
        <SynthesisModal
          ...
          authorName={perspectives.find((p) => p.id === activeSynthesisNode)?.author.name || ""}
          content={perspectives.find((p) => p.id === activeSynthesisNode)?.content || ""}
