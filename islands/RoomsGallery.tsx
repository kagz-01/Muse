    ...
    <>
      ...
      <div className="p-6 md:p-10 max-w-7xl mx-auto pb-24 md:pb-10">
        ...
        <header className="mb-10 flex items-end justify-between border-b border-white/5 pb-6">
          <div>
            <h1 className="text-4xl font-bold tracking-tight mb-2 text-white">Your Rooms</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {rooms.map((room) => {
            ...
            return (
              <a
                key={room.id}
                href={`/rooms/${room.id}`}
                className="relative h-64 rounded-4xl overflow-hidden cursor-pointer group shadow-xl border border-white/5 hover:border-white/20 transition-all duration-300 transform hover:-translate-y-1 block"
              >
                {room.coverImage ? (
                  <img src={room.coverImage} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 ease-out" alt="" />
                ) : (
          ...
          <button
            ...
            type="button"
            className="relative h-64 bg-transparent border-2 border-dashed border-white/10 rounded-4xl hover:border-white/25 hover:bg-white/2 transition-all duration-300 cursor-pointer flex flex-col items-center justify-center group w-full"
          >
