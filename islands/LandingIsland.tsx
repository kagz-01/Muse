...
const PILLARS = [
  ...
  { label: 'Contemplate', color: 'text-indigo-400', desc: 'Find your patterns.' }, // Replaced text-canvas-primary to match Tailwind
  ...
  return (
    <div className="bg-[#07070a] text-white font-sans overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        <div 
          className="absolute top-[-15%] left-[-15%] w-[70%] h-[70%] bg-indigo-500 blur-[150px] rounded-full opacity-5"
        />
        <div 
          className="absolute bottom-[-20%] right-[-15%] w-[60%] h-[60%] bg-violet-600 blur-[150px] rounded-full opacity-5"
        />
      </div>
      ...
      <header className="fixed top-0 left-0 w-full px-6 md:px-12 py-5 flex justify-between items-center z-[100] animate-in slide-in-from-top duration-500">
        <div className="flex items-center gap-3">
          <img src="/assets/muse-logo.png" alt="Muse" className="h-9 w-9 object-contain rounded-xl" />
      ...
      {/* SCROLL CONTAINER */}
      <div className="h-screen overflow-y-scroll overflow-x-hidden scroll-smooth" style={{ scrollbarWidth: 'none' }}>
        ...
        <section className="min-h-screen relative z-10 py-32 px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0 w-full h-full max-w-5xl mx-auto border border-white/5 rounded-[3rem] overflow-hidden">
            {PILLARS.map((p, i) => (
              <div
                key={p.label}
                className={`flex flex-col items-center justify-center text-center p-16 border-white/5 ${i % 2 === 0 ? 'md:border-r' : ''} ${i < 2 ? 'border-b md:border-b' : ''} relative group hover:bg-white/[0.02] transition-colors`}
              >
        ...
        <footer className="border-t border-white/5 py-10 px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-4 text-gray-600 text-[11px] font-bold uppercase tracking-widest z-10 relative">
          <div className="flex items-center gap-2">
            <img src="/assets/muse-logo.png" alt="Muse" className="h-6 w-6 object-contain rounded-lg opacity-60" />
            <span>Muse © 2025</span>
          </div>
