...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (isHovering) return; // Pause on hover or touch

    const interval = setInterval(() => {
      if (carouselRef.current) {
        const { scrollLeft, scrollWidth, clientWidth } = carouselRef.current;
        // If we reached the end of the scroll container
        if (scrollLeft + clientWidth >= scrollWidth - 10) {
          carouselRef.current.scrollTo({ left: 0, behavior: 'smooth' });
        } else {
          // Scroll right by roughly the width of one card + gap (280px + 24px)
          carouselRef.current.scrollBy({ left: 304, behavior: 'smooth' });
        }
      }
    }, 3500); // Nudge every 3.5 seconds
      ...
      <div className="max-w-7xl mx-auto pb-24 md:pb-10 min-h-full">
        ...
        <section className="px-6 md:px-10">
          ...
          <div
            ...
            onTouchEnd={() => {
              // slight delay before resuming to allow swipe momentum to settle
              setTimeout(() => setIsHovering(false), 2000);
            }}
            className="flex gap-6 overflow-x-auto pb-12 pt-4 snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
          >
            {rooms.map((room) => {
              ...
              return (
                <button
                  ...
                  className="relative h-56 w-[70vw] sm:w-[280px] shrink-0 snap-center sm:snap-start rounded-3xl overflow-hidden cursor-pointer group shadow-lg border border-white/5 hover:border-white/20 transition-all duration-500 transform hover:-translate-y-1 text-left"
                >
                  <img src={room.coverImage} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-[1.5s] ease-out" />
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-between">
                    ...
                    <div className="transform transition-transform duration-500 group-hover:translate-y-[-4px]">
                      <h3 className="text-2xl font-bold tracking-tight text-white mb-1.5 drop-shadow-lg group-hover:text-white transition-colors">
                        {room.name}
                      </h3>
                      <span className="text-[11px] font-bold uppercase tracking-widest text-[#a8a29e] drop-shadow-md">
            ...
            <button
              ...
              className="relative h-56 w-[70vw] sm:w-[280px] shrink-0 snap-center sm:snap-start bg-[#121212] border-2 border-dashed border-white/10 rounded-3xl hover:border-canvas-primary/30 hover:bg-white/2 transition-colors cursor-pointer flex flex-col items-center justify-center text-gray-500 hover:text-white group">
              <div className="w-12 h-12 rounded-full border border-dashed border-gray-600 group-hover:border-canvas-primary flex items-center justify-center mb-3 text-xl font-light transition-colors">
                +
              </div>
